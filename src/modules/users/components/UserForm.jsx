...
export default function UserForm({ user, packages = [], onSave, onClose }) {
  ...
  const handleExpirationChange = (value) => {
    setForm(current => ({
      ...current,
      expiresAt: value,
      status: current.status === 'Suspendido' ? 'Suspendido' : (value && value >= todayIso() ? 'Activo' : 'Vencido')
    }));
  };
  ...
  const submit = async (e) => {
    ...
    if (!username || !name || !form.packageId || !form.expiresAt) return setError('Usuario, nombre, paquete y vencimiento son obligatorios.');
    if (username.length < 3) return setError('El usuario debe tener al menos 3 caracteres.');
    if (maxConnections < 1 || maxConnections > packageMax) return setError(`El máximo de conexiones debe estar entre 1 y ${packageMax}.`);
    if (!user && password.length < 1) return setError('La contraseña es obligatoria y debe tener al menos 1 carácter.');
    setError('');
    const payload = { ...form, username, name, maxConnections, password };
    const result = await onSave(payload);
    if (result !== true) setError(result?.error || 'No se pudo guardar el cliente. Revisa los datos.');
  };

  return <div className="modal-backdrop"><form className="modal user-modal" onSubmit={submit}>
    <div className="modal-head"><div><h2>{user?'Editar cliente':'Nuevo cliente IPTV'}</h2><p className="modal-subtitle">Cuenta de cliente, seguridad y servicio contratado.</p></div><button type="button" className="icon-button" onClick={onClose}><X size={18}/></button></div>
